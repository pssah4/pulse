...
| `go_autostart` | `true` | the [live map](../guides/pulse-map#it-starts-pulse-go) starts `pulse go` apart from it when approved work waits (a build that starts next, or an approved item nobody holds that needs a PLAN) and no run of this clone lives, and only while this file equals the one on origin's default branch, or on the base branch that one names; `false` turns that off, as `PULSE_GO=off` in the environment does for one shell |
