...
- Board commands such as `pulse status` and `show` talk to GitHub, which the Codex sandbox blocks: approve them when Codex asks, or run `pulse setup --codex-rules` once (`pulse setup --remove --codex-rules` takes it back). With the rules, Codex still asks before `approve`, `approve-plan`, `rank`, `done`, `release`, and `claim`.
